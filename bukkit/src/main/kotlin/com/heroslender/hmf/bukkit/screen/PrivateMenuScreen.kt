...
class PrivateMenuScreen(
    ...
    private val owner = arrayOf(owner)
    private val cursor = Cursor(width * 128 / 2, height * 128 / 2, MapIcon.Type.GREEN_POINTER)

    override fun spawn() = chunks.forEach { it.create(owner) }

    override fun despawn() = chunks.forEach { it.destroy(owner) }
    ...
    override fun update(source: MapCanvas) {
        chunks.forEachIndexed { index, chunk ->
            ...
            chunk.updateBuffer(source.buffer, source.width, startX, startY, owner)
        }
    }
    ...
    override fun updateCursor(player: Player, x: Int, y: Int) {
        ...
        if (prevCursorChunk !== chunk) {
            prevCursorChunk?.sendCursorUpdate(players = owner)
        }
